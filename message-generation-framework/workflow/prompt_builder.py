...
def format_structured_human_feedback(human_feedback: Dict[str, Any]) -> str:
    """
    Format structured human feedback into a prompt-friendly format.
...
def create_single_message_prompt(
    ...
    if human_feedback:
        if isinstance(human_feedback, dict):
            formatted_human_feedback = format_structured_human_feedback(human_feedback)
        else:
...
def create_single_message_prompt(
    ...
    if current_iteration == 1:
        ...
        # This is a refinement iteration, so include previous message and feedback
        generation_prompt = f"""
        Refine the following message for the psychological concept "{concept_name}":
        ...
        IMPORTANT: Do NOT mention the name of the concept or any technical psychological terminology in your message. Use ordinary language that communicates the essence of the concept without naming it.
    ...
    # If there are previous final messages, make sure this one is different
    if final_messages:
        generation_prompt += "\n\nIMPORTANT: Create a message that differs from these previously accepted messages while maintaining alignment with the concept:"
        for i, prev_msg in enumerate(final_messages, 1):
            generation_prompt += f"\n{i}. \"{prev_msg}\""
